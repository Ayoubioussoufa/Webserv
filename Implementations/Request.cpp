...
/*   Updated: 2023/09/21 13:47:04 by aybiouss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int Request::parseHttpRequest(const std::string& requestBuffer, int new_socket) {
    ...
    static int i = 0;
    (void)new_socket;
    // exit(1); // ! if its chunked makikounch fiha content length ???? 
    if (requestBuffer.find("Transfer-Encoding:chunked") != std::string::npos && !i)
    {
        i++;
